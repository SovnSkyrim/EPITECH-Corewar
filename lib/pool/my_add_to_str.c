...
char **my_add_to_array(char **array, char *input)
{
    ...
    char **output = my_malloc(sizeof(char *) * (height + 1));
    output[height + 1] = NULL;
    for (int i = 0; array[i]; i++)
        output[i] = my_malloc(sizeof(char) * (my_strlen(array[i] + 1)));
    add_to_array_copy(output, array);
    output[height] = my_malloc(sizeof(char) * (len_input + 1));
...
char *my_add_to_str(char *base, char input)
{
    ...
    char *output = my_malloc(sizeof(char) * (length + 1));
    output[length] = '\0';
